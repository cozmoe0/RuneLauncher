...
import { Minus, Scaling, Square, X } from "lucide-react";
...
export function Titlebar() {
    ...
    const updateIsWindowMaximized = React.useCallback(async () => {
        try {
    ...
    return (
      <div data-tauri-drag-region className="flex flex-row inline-flex relative top-0 left-16 min-w-[calc(100%-3.8rem)] h-12 bg-slate-900/95 backdrop-blur-sm dropshadow-lg border-b border-slate-700 z-30 items-center -translate-x-1">
          <div data-tauri-drag-region className="flex flex-row py-4">
              <img data-tauri-drag-region src="icon.png" alt="" className="w-8 h-8 -translate-x-11 pointer-events-none select-none border-1 border-slate-700/90 rounded-lg dropshadow-lg" />
          </div>
          <div data-tauri-drag-region className="flex flex-row mx-0 px-1 -translate-x-6">
              <span data-tauri-drag-region className="text-slate-300 text-md font-medium tracking-wide select-none">
                  Rune<span data-tauri-drag-region className="text-emerald-400">Launcher</span>
              </span>
          </div>
          <div data-tauri-drag-region className="flex flex-1"></div>
          <div data-tauri-drag-region className="flex flex-row">
              <div className="flex items-center mr-1 space-x-1">
                  {/* Minimize Button */}
                  <motion.button
                      onClick={minimize}
                      className="w-8 h-8 rounded-sm flex items-center justify-center text-slate-400 hover:text-slate-200 hover:bg-slate-700/50 transition-colors duration-150"
                  ...
                  <motion.button
                      onClick={close}
                      className="w-8 h-8 rounded-sm flex items-center justify-center text-slate-400 hover:text-white hover:bg-red-600/50 transition-colors duration-150"
                      whileHover={{ scale: 1.05 }}
                      whileTap={{ scale: 0.95 }}
                      title="Close"
                  >
